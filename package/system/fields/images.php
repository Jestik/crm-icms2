...
class fieldImages extends cmsFormField {
    ...
    public function getOptions() {
        ...
        return [
            ...
            // НАЧАЛО: Наша новая опция для камеры
            new fieldCheckbox('use_camera', [
                'title' => 'Открывать камеру на мобильных'
            ]),
            // КОНЕЦ: Наша новая опция для камеры
    ...
    public function getInput($value) {
        ...
        $html = parent::getInput($value);

        // Добавление второй кнопки "Сделать фото"
        if ($this->getOption('use_camera')) {
            $html .= '<script>
                document.addEventListener("DOMContentLoaded", function() {
                    
                    // показываем вторую кнопку только на телефонах
                    if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia)
                    
                    var wrapperId = "f_'.$this->name.'";
                    var btnId = "cam_btn_" + wrapperId;
                    ...
                    setInterval(function() {
                        ...
                        if (!document.getElementById(btnId)) {
                            ...
                            var camBtnHtml = \'<div style="margin-top: 10px;">\' + 
                                             \'<button type="button" id="\'+btnId+\'" style="padding: 7px 14px; background: #888888; color: #fff; border: none; border-radius: 4px; cursor: pointer; font-size: 13px; font-weight: 500; display: inline-flex; align-items: center; gap: 6px; box-shadow: 0 2px 4px rgb(0 0 0 / 20%);">\' +
                                             \'📸 Сделать фото\' +
                                             \'</button></div>\';
                            
                            wrapper.insertAdjacentHTML("beforeend", camBtnHtml);
